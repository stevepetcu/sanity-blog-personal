    ...
    <div className={cn('group rounded')}>
      {coverImage && (
        <div className={cn('relative mb-2')}>
          ...
          <span
            className={cn(
              'absolute left-2 top-1 text-xl font-bold text-white/75 sm:text-sm lg:text-lg'
            )}
          >
            {index}/{total}
          </span>
      ...
      {!coverImage && (
        <div className={cn('pt-0 md:pt-0.5 lg:pt-1')}>
          <span
            className={cn(
              'ml-2 text-lg font-bold text-slate-400 sm:text-base lg:text-lg'
            )}
          >
            {index}/{total}
          </span>
      ...
      <div className={cn('p-2')}>
        <h3 className={cn('text-lg font-medium leading-snug text-slate-800')}>
        ...
        {!coverImage && (
          <Link href={`${POSTS_PAGE_PATH}/${slug}`} className="hover:underline">
            <p
              className={cn(
                'line-clamp-3 text-base leading-relaxed text-slate-500'
              )}
            >
              {summary}
            </p>
        ...
        <div className={cn('mt-2.5 flex flex-wrap text-sm text-slate-800')}>
        ...
        {tags && tags.length && (
          <div className={cn('mt-2.5 flex flex-wrap')}>
            <TagList
              tags={tags}
              itemClassNames={'mr-1.5 mb-1.5 text-xs sm:text-sm lg:text-base'}
            />
          </div>
        )}
      </div>
